...
// version SEED.md already states without the name: ananda is "never a gate,
// never a score: the warmth you check for." The moment anything prefers high
// ananda, or treats `opened === true` as the better outcome, the vital sign
...
// failure this guards against: an ananda-ranked register put "candidates
// sitting at exactly minArrivals" on top, because a small extent gives a wide
// interquartile ground — ranking by ananda ranks by rarity, and the fix was
...
const nikantiViolations = (code) => {
  ...
  lines.forEach((line, i) => {
    ...
    // ananda; anything else is an invented "prefer this much or more" gate.
    if (/(\bananda\b|\bvolume\([^()]*\))\s*(>=?|<=?)\s*-?(?!0\b)\d/.test(line)) hits.push({ i, line, why: "threshold gate on the sign" });
    ...
    if (/\.sort\(/.test(line)) {
      ...
      if (/\bananda\b/.test(windowText) || /\.opened\b/.test(windowText))
...
test("the grep is not vacuous — a planted violation of each shape is caught", () => {
  const threshold = `const pick = ananda > 1.5 ? candidateA : candidateB;`;
  const branch = `if (record.opened === true) { promote(record); } else { discard(record); }`;
  const rank = `const ranked = candidates.sort((a, b) => b.ananda - a.ananda);`;
  ...
  assert.ok(nikantiViolations(rank).length > 0, "a sort keyed by ananda must be caught");
  ...
  assert.equal(nikantiViolations(`return { viable: room > 0, ananda: room };`).length, 0);
  assert.equal(nikantiViolations(`opened: closing.ananda > anandaAtOpen,`).length, 0);
});
