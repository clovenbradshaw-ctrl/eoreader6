...
test("existence-dependency: a burst regime shifts the ground", () => {
  ...
  const result = existenceDependencyTest(series, regime, { reseeds: 8 });
...
test("classify: burst regime is above, uniform is peer", () => {
  ...
  const eBurst = existenceDependencyTest(burstSeries, { start: 10, end: 15 }, { reseeds: 8 });
  ...
  const eUniform = existenceDependencyTest(uniformSeries, { start: 3, end: 8 }, { reseeds: 8 });
