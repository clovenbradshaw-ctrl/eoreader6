...
Example (for the task "units are sentences; series is characters per sentence"):
{"load_body": "return text.split(/[.!?]+/).filter(s => s.trim().length > 0);", "unit_count_expr": "units.length", "reduce_body": "return units.slice(0, readTo).map(s => s.length);"}`;
